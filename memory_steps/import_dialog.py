...
class ImportDialog(QDialog):
    def __init__(self,parent=None):
        ...
        self.profile=QComboBox(); profiles=cfg.get("anchor_profiles",{}) or {"General / Mixed Text":[]}; [self.profile.addItem(n) for n in profiles.keys()]; self.profile.setCurrentText(cfg.get("default_anchor_profile","General / Mixed Text"))
        self.window=QSpinBox(); self.window.setRange(1,5); self.window.setValue(1); self.auto=QCheckBox("Automatically split numbered text into lines"); self.auto.setChecked(True)
    ...
    def import_text(self):
        ...
        for item in data:
            note=mw.col.new_note(model); values=list(item.__dict__.values())
