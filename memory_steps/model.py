...
/* Mobile layout: compact controls above a scrollable prompt area. */
@media (max-width:640px){
.card{padding:8px 8px 92px 8px;font-size:17px;line-height:1.45}.wrap{max-width:none;width:100%;margin:0}.title,.meta,.instructions,.keyboard-help,.context{display:none!important}.reference{font-size:18px;padding:3px 10px;margin:2px 0 8px 0}.player{display:flex;flex-direction:column;padding:8px;margin:0;border-radius:14px;max-height:calc(100vh - 160px);overflow:hidden}.player-top{order:1;margin-bottom:6px;gap:4px}.badge{font-size:12px;padding:3px 8px}.controls{order:2;grid-template-columns:repeat(5,minmax(0,1fr));gap:4px;margin-bottom:6px}.ms-btn{font-size:12px;line-height:1.15;padding:7px 4px;min-height:38px;border-radius:9px}.step-title{order:3;font-size:14px;line-height:1.25;margin:2px 0 4px 0}.player .versebox{order:4;margin:0;padding:12px;border-radius:12px;min-height:0;max-height:calc(100vh - 300px);overflow-y:auto;-webkit-overflow-scrolling:touch}.versebox.layout-cjk{font-size:clamp(24px,8vw,34px);line-height:1.55;word-break:break-word;overflow-wrap:anywhere}.versebox.layout-latin{font-size:clamp(20px,6vw,28px);line-height:1.45;overflow-wrap:anywhere}.versebox.layout-mixed{font-size:clamp(21px,7vw,30px);line-height:1.5;overflow-wrap:anywhere}details.hint,.fallback{display:none!important}}
"""
...
(function(){
  ...
  function render(){
    ...
    if(checking){
      ...
      hintBtn.textContent='Back'; harderBtn.textContent=advanceAfterCheck?'Continue →':'Back'; checkBtn.textContent=advanceAfterCheck?'Continue':'Back'; return;
    }
    ...
    hintBtn.textContent=mode==='recall'?'Hint ←':'Easier ←'; harderBtn.textContent=mode==='recall'?'Harder →':'Next →'; checkBtn.textContent='Check';
  }
  ...
  function toggleCheck(){if(checking){advanceAfterCheck ? continueToNext() : backToStep(); return;} checking=true; advanceAfterCheck=false; render();}
  function nextTrain(){if(checking){advanceAfterCheck ? continueToNext() : backToStep(); return;} if(mode==='train'){checking=true; advanceAfterCheck=true; render(); return;} harder();}
  ...
  hintBtn.addEventListener('click',easier); harderBtn.addEventListener('click',nextTrain); checkBtn.addEventListener('click',toggleCheck);
  render();
})();
</script>
"""
